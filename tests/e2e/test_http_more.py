...
async def test_study_plan_validate_and_delete_item(client) -> None:  # type: ignore[no-untyped-def]
    # dry-run 校验
    r = await client.post(
        "/api/course-selection/v1/study-plans/me/validate",
        headers=_STU,
        json={
            "major_code": "CS",
            "curriculum_version": "2023",
            "items": [
                {"course_code": "CS101", "category": "major_required", "expected_semester": "2026-1", "credit": 9}
            ],
        },
    )
    ...
    d = await client.delete(f"/api/course-selection/v1/study-plans/me/items/{uuid.uuid4()}", headers=_STU)
...
async def test_ai_conversation_and_recommendation(client) -> None:  # type: ignore[no-untyped-def]
    ...
    rec = await client.post(
        "/api/course-selection/v1/ai/recommendations", headers=_STU, json={"goal": "补满专业选修", "semester": "2026-1"}
    )
...
async def test_cross_team_student_enrollments_self_and_forbidden(client) -> None:  # type: ignore[no-untyped-def]
    # 本人可查（空列表也 200）
    ok = await client.get(
        "/api/course-selection/v1/students/S-more/enrollments", headers=_STU, params={"semester": "2026-1"}
    )
    assert ok.status_code == 200
    # 学生查他人 → 403
    forbid = await client.get(
        "/api/course-selection/v1/students/OTHER/enrollments", headers=_STU, params={"semester": "2026-1"}
    )
