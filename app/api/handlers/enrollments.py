...
async def create_enrollment(
    ...
    outcome = await service.enroll(
        principal, student_id=principal.user_id, offering_id=body.offering_id,
        stage=body.stage, idempotency_key=body.idempotency_key,
    )
...
async def delete_enrollment(
    enrollment_id: str, principal: CurrentUser, service: EnrollmentServiceDep
) -> Response:
...
async def my_enrollments(
    ...
    views = [
        EnrollmentView(
            ...
            course_code=off.course_code if off else "", course_name=off.course_name if off else "",
            teacher_id=off.teacher_id if off else "", teacher_name=off.teacher_name if off else "",
            status=e.status, stage=e.stage, enrolled_at=e.enrolled_at,
        )
        for e, off in rows
    ]
...
async def queue_position(
    principal: CurrentUser, offering_id: str = Query(...)
) -> Envelope[QueuePosition]:
    principal.require_role(Role.STUDENT)
